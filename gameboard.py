...
class GameBoard:
    ...
    def __init__(self, size):
        """
        Constructor: it needs a Shape(width, height) gameboard is based in 3 parameters, two integers: height, width and a matrix: board which represents the gameboard.
    ...
    # It makes it in O(h), where h is the board height. I use a vector which is a complete row,
    ...
    # It makes it in O(w), where w is the board width, to do so, 
    # I have to transpose the board in order to get its rows, 
    # that's because it isn't possible to acces by columns in a matrix.
    ...
    # O(number_of_rows*width)
    def clear_rows(self, rows):
        """ 
        This method erases the tokens of the given rows, rows is a vector which contains the rows which are going to be cleared.
    ...
    # O(number_of_columns*height)
    def clear_columns(self, columns):
        """ 
        This method erases the tokens of the given columns, columns is a vector which contains the rows which are going to be cleared.
    ...
    # O(rectangle.height*rectanlge.width)
    def put(self, location, rectangle = Shape(1, 1)):
        """ 
        This method puts, if it's possible, a rectangle of tokens, which has its low-left corner in location if the rectangle is not specified, the method will place only one token, in the location.
        """
        try:
    ...
    def remove(self, location,rectangle = Shape(1, 1), must_full = True):
        """ 
        This method erases, if it's possible; that is, if the rectangle is fully occuped and it fits in the board, a rectangle of tokens, which has its low-left corner in location if the rectangle is not specified, the method will erase only one token, in the location.
        """
        size = self.get_shape()
        if must_full:
            try:
    ...
    # O(rectangle.width*rectangle.height)
    def is_empty(self,location, rectangle=Shape(1,1)):
        """ 
        This method returns wether a rectangle is empty, the rectangle is specified by its shape and location is the low-left corner. If the rectangle is not specified, it considers only the location of low-left corner.
        """
        size = self.get_shape()
        location = self.__transform_coordinates(location, size)
        for i in range(rectangle.height):
    ...
    def is_full(self,location, rectangle=Shape(1,1)):
        """
        This method returns wether a rectangle is full, the rectangle is specified by its shape and location is the low-left corner. If the rectangle is not specified, it considers only the location of low-left corner.
        """
        size = self.get_shape()
        location = self.__transform_coordinates(location, size)
        for i in range(rectangle.height):
    ...
    def __repr__(self):
        """
        This method changes the data shown by the type GameBoard, the data that must be shown is, the shape, and the squares with token.
        """
        size = self.get_shape()
    ...
    def row_counters(self):
        """
        This method counts how many tokens are in every row, it returns it in a list, in which every component of the list represent a row.
        """
        size = self.get_shape()
        row_counter = []        
    ...
    def column_counters(self):
        """
        This method counts how many tokens are in every column, it returns it in a list, in which every component of the list represents a column.
        """
        size = self.get_shape()
        column_counter = []        
        for j in range(size.width):
    ...
    # O(1)
    # I use this method in order to adapt the coordinates to the specified.
    def __transform_coordinates(self, location, size):
        """ 
        This private method, transform the coordinates of a given location to the coordinates that are being used to identify the rows and columns most methods use this one. 
        """
